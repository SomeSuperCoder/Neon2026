...
- [x] 1. Add Go serialization helpers for DPoS data models
  ...
  - ✅ COMPLETED: All serialization functions implemented with proper error handling

- [x] 1.1 Write unit tests for serialization round-trips
  - Create `internal/quanticscript/stdlib_staking_test.go` covering all four data model types
  - _Requirements: 6.1, 10.1_
  - ✅ COMPLETED: All tests passing (ValidatorRecord, StakeAccount, EpochState, RewardPool round-trips + error cases)

- [x] 2. Implement the Staking Program in QuanticScript
- [x] 2.1 Write `programs/staking/staking.qs` — entry dispatch and validator registration handlers
...
- [x] 2.2 Add stake delegation handlers to `programs/staking/staking.qs`
...
- [x] 2.3 Add reward distribution handler to `programs/staking/staking.qs`
  - Implement `handleDistributeRewards` — proportional reward split, commission deduction, fractional remainder retention
  - _Requirements: 4.1, 4.2, 4.3, 4.4, 4.5_

- [x] 2.4 Add slashing handler to `programs/staking/staking.qs`
...
- [x] 2.5 Compile `staking.qs` to `staking.qsa` and `staking.qsb`
...
- [x] 2.6 Write QuanticScript interpreter tests for each instruction type
  - Create `internal/quanticscript/staking_program_test.go` exercising all 7 handlers against a mock ExecutionContext
  - _Requirements: 6.3, 6.4, 6.5, 6.6_

- [x] 3. Extend genesis bootstrap to load the Staking Program and initialize DPoS state
- [x] 3.1 Update `internal/genesis/programs.go` — add `stakingBytecode` parameter to `LoadBuiltinPrograms`
  - Load `staking.qsb` at `StakingProgramID` using the existing `loadProgram` helper
  - _Requirements: 6.5, 9.1_
  - ✅ COMPLETED: Updated LoadBuiltinPrograms signature to accept optional stakingBytecode; updated all callers (cmd/main.go + 5 integration tests)

- [x] 3.2 Implement `InitializeDPoSGenesis` in `internal/genesis/programs.go`
...
- [x] 3.3 Update `programs/embed.go` to embed `programs/staking/staking.qsb`
...
- [x] 3.4 Write genesis integration tests
...
- [x] 4. Extend ConsensusManager with DPoS scheduling and epoch processing
- [x] 4.1 Add epoch fields and `GenesisConfig` to `internal/consensus/consensus_manager.go`
...
- [x] 4.2 Implement `InitializeGenesis` on `ConsensusManager`
...
- [x] 4.3 Implement stake-weighted validator schedule computation
...
- [x] 4.4 Implement `IsLeader`, `GetScheduledValidator`, and slot-skip logic
...
- [x] 4.5 Implement `ProcessEpochBoundary` — activate/deactivate validators and trigger reward distribution
...
- [x] 4.6 Implement `RecordMissedBlock` — update missed-block counter in Validator Record and Epoch State File
  - _Requirements: 3.6, 9.4_

- [x] 4.7 Write ConsensusManager unit tests
